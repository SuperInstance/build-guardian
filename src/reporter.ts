...
import type { BuildReport, BloatAlert } from './types';
...
export function generateReport(report: BuildReport): string {
  ...
  lines.push('');
  lines.push('| Entry | Build Time | Output Size | Memory Peak |');
  lines.push('|-------|-----------:|------------:|------------:|');
  for (const m of report.entryMetrics) {
    lines.push(`| ${m.entry} | ${formatDuration(m.buildTimeMs)} | ${formatBytes(m.sizeBytes)} | ${formatBytes(m.memoryPeakBytes)} |`);
  }
  lines.push('');
